...
public class CopyOfImageUtil {
	...
	private static final Log log = LogFactory.getLog(CopyOfImageUtil.class);
	 ...
	 * @return
	 */
	public static String reuceImgOnlyWidth(File srcfile,String parentPath, int widthdist) {
		try {
...
//				encoder.encode(tag);
			...
			String temp = parentPath.substring(0,parentPath.lastIndexOf("/")) + "/"+ widthdist +"/" +  fileName;
			return temp;
		} catch (Exception e) {
			log.error(e);
			return null;
...
//				encoder.encode(tag);
	 ...
	 * @return
	 */
	public static String reuceImgCutPart(File srcfile,String parentPath, int x, int y, int width, int height) {
		try {
			String filedir = srcfile.getParent();
			String fileName = System.currentTimeMillis() + parentPath.substring(parentPath.lastIndexOf("."));
...
//			encoder.encode(tag);
			...
			return parentPath.substring(0,parentPath.lastIndexOf("/")) + "/" + width + "_" + height +"/" +  fileName;
		} catch (Exception e) {
			log.error(e);
			return null;
		}
